package UserView;

import java.util.InputMismatchException;
import java.util.Scanner;
import Service.Authenticator;

public abstract class Menu
{
    protected static Scanner sc = new Scanner(System.in);
    protected static Authenticator authenticator = new Authenticator();

    public Menu() { sc = new Scanner(System.in);}
    public static void start() {}

    //helper function to get repeated until valid int choice with repeat message
    public static int GetIntInput(String repeat)
    {
        int choice = -1;

        while (choice < 0) {
            try {
                System.out.print(repeat);
                choice = sc.nextInt();
            } catch (final InputMismatchException e) {
                System.out.println("\nError: Invalid input");
                sc.nextLine(); // ignore and move the cursor to next line
            }
        }

        return choice;
    }

    //helper function to get repeated until valid int choice with repeat message (with the exception of -1)
    public static int GetIntInputAlt(String repeat)
    {
        int choice = -99;

        while (choice < 0) {
            try {
                System.out.print(repeat);
                choice = sc.nextInt();
                if (choice == -1) { return choice; }
            } catch (final InputMismatchException e) {
                System.out.println("\nError: Invalid input");
                sc.nextLine(); // ignore and move the cursor to next line
            }
        }

        return choice;
    }

    //helper function to get repeated until valid double choice with repeat message (with the exception of -1)
    public static double GetDoubleInput(String repeat)
    {
        double choice = -99;

        while (choice < 0) {
            try {
                System.out.print(repeat);
                choice = sc.nextDouble();
                if (choice == -1) { return choice; }
            } catch (final InputMismatchException e) {
                System.out.println("\nError: Invalid input");
                sc.nextLine(); // ignore and move the cursor to next line
            }
        }

        return choice;
    }

    //helper function to get repeated until valid string with repeat message
    public static String GetStringInput(String repeat)
    {
        String choice = "";

        while (true) {
            try {
                System.out.print(repeat);
                choice = sc.nextLine();
            } catch (final InputMismatchException e) {
                System.out.println("\nError: Invalid input");
                continue;
            }
            break;
        }

        return choice;
    }

}
